...
class CocoAnnotations:
  ...
  def get_image_dict(self, img_name):
    image_hash = int(int(hashlib.sha256(img_name.encode('utf-8')).hexdigest(), 16) % sys.maxsize)
  ...
  def read_file(self,filename):
    ...
    with open(filename,'r') as opfd:
      for line in opfd:
        ...
        sent = id_sent[1]    #.encode('ascii', 'ignore').decode('ascii')
  ...
  def dump_json(self, outfile):
    ...
    with io.open(outfile, 'w', encoding='utf-8') as fd:
      fd.write(json.dumps(res, ensure_ascii=False, sort_keys=True, indent=2, separators=(',', ': ')))
