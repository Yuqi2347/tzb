...
for task, tasks_values in results.items():
    ...
    for substask, subtask_value in tasks_values.items():
        ...
        if task=="Perception" and substask=="Remote Sensing":
            acc_rs = acc_subtasks
...
if "CN" in args.results_file:
    lan = "CN"
else:
    lan = "EN"
print(f'*'*64 + f'Overall Acc_RS_{lan}: ' + '{:.4f}'.format(acc_subtasks) +f'*'*64)    
# print(f'*'*64 + f'Overall Acc: ' + '{:.4f}'.format(succ_all / sum_all) +f'*'*64)