    ...
    def _load_image(self, image_file):
        ...
        if image_file.lower().endswith(('.tif', '.tiff')):
            ...
            try:
                ...
                logger.warning(f"tifffile failed for {image_file}: {e}")
            ...
            if image is None:
                try:
                    ...
                    logger.warning(f"cv2 failed for {image_file}: {e}")
            ...
            if image is None:
                try:
                    ...
                    logger.warning(f"PIL relaxed mode failed for {image_file}: {e}")
        ...
        if image is None:
            try:
                ...
                logger.error(f"All methods failed to load {image_file}: {e}")
                # 创建一个空白图片作为fallback
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            if isinstance(self.raw_data[i]["image"], str):
                images_dict = { "<image>" : self._load_image(os.path.join("",self.raw_data[i]["image"])) }
                ...
                images_dict = {img_name : self._load_image(img_path) for img_name, img_path in self.raw_data[i]["image"].items()}
            ...
            ret = dict(
                ...
                is_cot_sample=ret["is_cot_sample"],
            )
        except Exception as e:
...
def data_collator(examples, padding_value=0, max_length=2048):
    # 恢复原始逻辑：只填充到 batch 内最大长度（不强制填充到 max_length）
    ...
    is_cot_sample = [example["is_cot_sample"] for example in examples]
    return {
        "input_ids": input_ids,
...
def conversation_to_ids_model(conversation, tokenizer):
    system_prompt = """You are a remote sensing assistant. Think step by step using <think>...</think> and give your answer in <answer>...</answer>."""
    raw_msg = ""
    chat = []
    context = []
    ...
    # print(f"inputs :{ret}")
    input_ids = tokenizer(ret)['input_ids'] # add bos_token first
...
def preprocess(
    ...
    assert conversations[0]["from"] == "human", "the first role must be user"
    ...
    # print(f"slice_config: {slice_config}")
    ...
    if batch_vision:
        tgt_sizes = []
        reshape_images = []
        # print(f"len of image:{len(images)}")
        for image in images:
        ...
        # print(f"len of reshape_images:{len(reshape_images)}")
    ...
    input_dict["is_cot_sample"] = True if "<think>" in str(conversations[1]["value"]) else False
    return input_dict
